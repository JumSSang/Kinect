...
// delete[] g_pBoneMatrices; 안지우고 있으니 알아서 맘데로 하세영.

void RenderMark(IDirect3DDevice9* Device,const D3DXVECTOR3 &vPos,DWORD dwColor)
{
	struct VB
	{
		D3DXVECTOR3 vPos;
	...
	vb[0].vPos = vPos+D3DXVECTOR3(5,0,0);
	vb[1].vPos = vPos+D3DXVECTOR3(-5,0,0);
	vb[2].vPos = vPos+D3DXVECTOR3(0,5,0);
	vb[3].vPos = vPos+D3DXVECTOR3(0,-5,0);
	vb[4].vPos = vPos+D3DXVECTOR3(0,0,5);
	vb[5].vPos = vPos+D3DXVECTOR3(0,0,-5);
	for(int i=0;i<6;i++) vb[i].dwColor = dwColor;

	D3DXMATRIX identity;
	D3DXMatrixIdentity(&identity);
	Device->SetRenderState(D3DRS_ZENABLE,FALSE);
	Device->SetTransform(D3DTS_WORLD,&identity);
	Device->DrawPrimitiveUP(D3DPT_LINELIST,3,vb,sizeof(VB));
...
{
	/*주석처리 부분
	스킨드메쉬가 하나라도 지워지면 복제된 스킨드메쉬도 같이 지워져버림.
	그것을 방지하기 위해 주석처리 해둠.
	*/
...
// Desc: 매쉬를 그린다.
//--------------------------------------------------------------------------------------
void CSkinnedMesh::MeshRender(D3DXMATRIXA16* _matWorld, float timeElapsed)
{
...
HRESULT CSkinnedMesh::Clone(const CSkinnedMesh * pFrom)
{
	ID3DXAnimationController *pAnim = pFrom->m_pAnimController;

	int nOutputs = pAnim->GetMaxNumAnimationOutputs();
	int nSets = pAnim->GetMaxNumAnimationSets();
	int nMaxTracks = pAnim->GetMaxNumTracks();
	int nMaxEvents = pAnim->GetMaxNumEvents();

	HRESULT hr = pAnim->CloneAnimationController(nOutputs,nSets,nMaxTracks,nMaxEvents,&m_pAnimController);
	if(FAILED(hr))
	{
		MessageBox(NULL,"복제 실패 ㅠㅠ",NULL,MB_OK | MB_ICONERROR);
		return NULL;
	}

	m_pD3DDevice = pFrom->m_pD3DDevice;
	m_pD3DDevice->AddRef();

	m_pEffect = pFrom->m_pEffect;
	if(m_pEffect != NULL) m_pEffect->AddRef();

	m_pFrameRoot = pFrom->m_pFrameRoot;
	m_vObjectCenter = pFrom->m_vObjectCenter;
	m_fObjectRadius = pFrom->m_fObjectRadius;
	m_SkinningMethod = pFrom->m_SkinningMethod;

	for(int i=0;i<4;i++)
	{
		LPDIRECT3DVERTEXSHADER9 pShader = m_pIndexedVertexShader[i] = pFrom->m_pIndexedVertexShader[i];
		if(pShader != NULL) pShader->AddRef();
	}
	m_filename = pFrom->m_filename;
...
HRESULT CSkinnedMesh::LoadMesh(LPDIRECT3DDEVICE9 pd3dDevice, const std::string &filename)
{
	...
	if(FAILED(D3DXLoadMeshHierarchyFromX(m_filename.c_str(), D3DXMESH_MANAGED, pd3dDevice,
...
D3DXVECTOR3 CSkinnedMesh::GetInitialCameraPosition() const
{
	D3DXVECTOR3 cameraPos(0.0f,m_vObjectCenter.y-0.5f,-(m_fObjectRadius*2));
	return cameraPos;
}

D3DXMATRIX CSkinnedMesh::BoneMatrix(TCHAR* boneName)
{
	//D3DXMATRIX& boneMatrix;
	D3DXFRAME_DERIVED *r = (D3DXFRAME_DERIVED*)D3DXFrameFind( m_pFrameRoot, boneName );
	D3DXMATRIXA16 boneMatrix = r->CombinedTransformationMatrix;
	return boneMatrix;
}

void CSkinnedMesh::SetAnimationPlay(unsigned int index)
{
	ID3DXAnimationSet*  tAnim=NULL;

	m_pAnimController->GetAnimationSet(index, &tAnim);
	m_pAnimController->SetTrackAnimationSet( 0, tAnim );
	...
	if (pMeshContainer->pSkinInfo != NULL)
	{
		...
		else if (m_SkinningMethod == D3DINDEXEDHLSLVS) 
		{
			...
			{ 
				...
				m_pEffect->SetMatrixArray( "mWorldMatrixArray", g_pBoneMatrices, pMeshContainer->NumPaletteEntries);
...
std::string CSkinnedMesh::GetAnimationSetName(unsigned int index)
{
	...
	m_pAnimController->GetAnimationSet(index, &set );
